...
- **Shell bootstrap on Omarchy:** HM reaches bash via exactly one line at the *bottom* of `~/.bashrc`: `. "$HOME/.config/hm/bashrc.sh"`. That HM-generated file (from `aliases.nix`) sources `hm-session-vars.sh` (`NH_FLAKE`, `NIX_HOME`, PATH), then `~/.config/hm/aliases.sh`, then `~/.config/hm/ssh-prompt.sh` if present (written by `home-omarchy.nix`: a right-aligned `user@host` above the bash prompt when `SSH_CONNECTION` is set, since bash has no right prompt; the fish equivalent is a `fish_right_prompt` in the same file, and HM-managed starship hosts get it from `right_format` in `common.nix`). It runs after Omarchy's own `default/bash/rc`, so our aliases override Omarchy's on clashes (`c` = clear, not opencode) — intentional. Caveat: the Arch/Omarchy `.bashrc` returns early for non-interactive shells, so `ssh host 'cmd'` won't see these vars or aliases (call `~/.nix-profile/bin/nh` by full path there) — interactive terminals do.
