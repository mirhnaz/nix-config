...
BarWidget {
  ...
  component IndicatorLoader: Item {
    ...
    function injectProps() {
      ...
      // Local change: render indicator glyphs at the same size and slot as the
      // other bar icon buttons (BarIndicator defaults to the smaller caption font).
      if ("fontSize" in target) target.fontSize = Qt.binding(function() { return Style.bar.iconFont })
      if ("fixedWidth" in target) target.fixedWidth = Qt.binding(function() { return target.vertical ? -1 : Style.bar.iconSlot })
    }
